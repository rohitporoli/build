...
EOF
    ...
    __print_cardinal_functions_help
...
EOF
    T=$(gettop)
    for i in `cat $T/build/envsetup.sh $T/vendor/cardinal/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      echo "$i"
    done | column
}
...
function build_build_var_cache()
{
    ...
    cached_vars=`cat $T/build/envsetup.sh $T/vendor/cardinal/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
    cached_abs_vars=`cat $T/build/envsetup.sh $T/vendor/cardinal/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^cardinal_") ; then
       CARDINAL_BUILD=$(echo -n $1 | sed -e 's/^cardinal_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $CARDINAL_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       CARDINAL_BUILD=
    fi
    export CARDINAL_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the Cardinal-AOSP github
        T=$(gettop)
        pushd $T > /dev/null
        vendor/cardinal/build/tools/roomservice.py $product
        popd > /dev/null
        check_product $product
    else
        vendor/cardinal/build/tools/roomservice.py $product true
    fi
...
# Execute the contents of any vendorsetup.sh files we can (exluding generic vendorsetups) find.
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/cardinal/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
...
. vendor/cardinal/build/envsetup.sh
