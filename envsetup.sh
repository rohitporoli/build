...
function print_lunch_menu()
{
    ...
    if [ "z${CARDINAL_DEVICES_ONLY}" != "z" ]; then
       echo "... and don't forget the wings!"
    fi
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the CARDINAL github
        T=$(gettop)
        pushd $T > /dev/null
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        ZIPFILE=$CARDINAL_MOD_VERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.cardinal.device | grep -q "$CARDINAL_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $CARDINAL_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.cardinal.device | grep -q "$CARDINAL_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
